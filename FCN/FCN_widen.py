...
def model1(pretrained_weights=None, input_size=(256, 256, 6)):
    ...
    conv5 = Conv2D(1536, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv4)
    conv6 = Conv2D(1536, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    conv7 = Conv2D(1, 1)(conv6)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_1')
...
def model2(pretrained_weights=None, input_size=(256, 256, 6)):
    ...
    conv7 = Conv2D(1, 1)(conv6)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_2')
...
def model3(pretrained_weights=None, input_size=(256, 256, 6)):
    ...
    conv1 = Conv2D(512, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(512, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    v_x_f_1 = Lambda(lambda b: concatenate([tf.slice(b[1], [0, 0, 0, int(i * 512 / 5)], [-1, -1, -1, int(512 / 5)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(5)], axis=-1))([conv2, velocities])
    ...
    conv5 = Conv2D(1536, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_x_f_2)
    conv6 = Conv2D(1536, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    v_x_f_3 = Lambda(lambda b: concatenate([tf.slice(b[1], [0, 0, 0, int(i * 1536 / 5)], [-1, -1, -1, int(1536 / 5)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(5)], axis=-1))([conv6, velocities])
    concat_1 = concatenate([v_x_f_1, v_x_f_2, v_x_f_3], axis=-1)
    conv7 = Conv2D(1, 1)(concat_1)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_3')
...
def model4(pretrained_weights=None, input_size=(64, 64, 5)):
    ...
    conv1 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(inputs)
    conv2 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    ...
    conv7 = Conv2D(1, 1)(conv6)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_4')
...
def model5(pretrained_weights=None, input_size=(64, 64, 5)):
    ...
    conv1 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    conv3 = Conv2D(512, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv2)
    conv4 = Conv2D(512, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv3)
    conv5 = Conv2D(1024, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv4)
    ...
    conv7 = Conv2D(1, 1)(conv6)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_5')
...
def model6(pretrained_weights=None, input_size=(64, 64, 5)):
    ...
    conv1 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    v_x_f_1 = Lambda(lambda b: concatenate([tf.slice(b[0], [0, 0, 0, int(i * 256 / 4)], [-1, -1, -1, int(256 / 4)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(4)], axis=-1))([conv2, velocities])
    ...
    conv6 = Conv2D(1024, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    v_x_f_3 = Lambda(lambda b: concatenate([tf.slice(b[0], [0, 0, 0, int(i * 1024 / 4)], [-1, -1, -1, int(1024 / 4)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(4)], axis=-1))([conv6, velocities])
    concat_1 = concatenate([v_x_f_1, v_x_f_2, v_x_f_3], axis=-1)
    conv7 = Conv2D(1, 1)(concat_1)
    out = Add()([input, conv7])
    model = Model(inputs=inputs, outputs=out, name='FCN_widen_6')
