...
def FCN3(pretrained_weights=None, input_size=(256, 256, 5)):
    ...
    conv6 = Conv2D(1536, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    v_x_f_3 = Lambda(lambda b: concatenate([tf.slice(b[1], [0, 0, 0, int(i * 1536 / 4)], [-1, -1, -1, int(1536 / 4)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(4)], axis=-1))([conv6, velocities])
...
def FCN6(pretrained_weights=None, input_size=(64, 64, 5)):
    ...
    conv1 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(256, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    v_x_f_1 = Lambda(lambda b: concatenate([tf.slice(b[1], [0, 0, 0, int(i * 256 / 4)], [-1, -1, -1, int(256 / 4)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(4)], axis=-1))([conv2, velocities])
