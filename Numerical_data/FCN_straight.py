...
def model4(pretrained_weights=None, input_size=(64, 64, 3)):
    ...
    conv1 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(inputs)
    conv2 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    conv3 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv2)
    conv4 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv3)
    conv5 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv4)
    conv6 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    conv7 = Conv2D(1, 1)(conv6)
    out = Add()([input, conv7])
...
def model5(pretrained_weights=None, input_size=(64, 64, 3)):
    ...
    conv1 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    conv3 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv2)
    conv4 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv3)
    conv5 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv4)
...
def model6(pretrained_weights=None, input_size=(64, 64, 3)):
    ...
    conv1 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_times_in)
    conv2 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv1)
    v_x_f_1 = Lambda(lambda b: concatenate([tf.slice(b[0], [0, 0, 0, int(i * 768 / 2)], [-1, -1, -1, int(768 / 2)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(2)], axis=-1))([conv2, velocities])
    conv3 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_x_f_1)
    conv4 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv3)
    v_x_f_2 = Lambda(lambda b: concatenate([tf.slice(b[0], [0, 0, 0, int(i * 768 / 2)], [-1, -1, -1, int(768 / 2)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(2)], axis=-1))([conv4, velocities])
    conv5 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(v_x_f_2)
    conv6 = Conv2D(768, 3, activation='tanh', padding='same', kernel_initializer='he_normal')(conv5)
    v_x_f_3 = Lambda(lambda b: concatenate([tf.slice(b[0], [0, 0, 0, int(i * 768 / 2)], [-1, -1, -1, int(768 / 2)]) * tf.slice(b[1], [0, 0, 0, i], [-1, -1, -1, 1]) for i in range(2)], axis=-1))([conv6, velocities])
    concat_1 = concatenate([v_x_f_1, v_x_f_2, v_x_f_3], axis=-1)
    conv7 = Conv2D(1, 1)(concat_1)
    out = Add()([input, conv7])
